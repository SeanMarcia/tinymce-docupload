(function() {
  tinymce.PluginManager.requireLangPack('docupload');

  tinymce.create('tinymce.plugins.DocUpload', {
    DocUpload: function(ed, url) {
      var form,
          iframe,
          win,
          throbber,
      ...
      function showDialog() {
        win = editor.windowManager.open({
          ...
          body: [
            ...
            {type: 'textbox', name: 'alt',  label: ed.translate('Document Title')},
        ...
        win.off('submit');
        win.on('submit', insertDoc);

        /* WHY DO YOU HATE <form>, TINYMCE!? */
        iframe = win.find("iframe")[0];
        form = createElement('form', {
          action: ed.getParam("uploaddoc_form_url", "/tinymce_documents"),
          target: iframe._id,
          method: "POST",
        ...
        form.appendChild(createElement('input', {type: 'hidden', name: 'hint', value: ed.getParam("uploaddoc_hint", "")}));
